...
A model in Sequelize represents a table in the database. A model also predefines how data should be stored, and determines whether or not a database entry is valid (for example, if the entry should be a number, string, etc.).

Define a Model
Models are often defined as JavaScript classes. Each model (or class) represents a thing that your application will be working with – all of the nouns. A model contains a collection of attributes (or class properties) that are used to describe each model instance. For example, a "Person" model might include "name" and "age" attributes to represent the name and age of a specific person.

Let's begin to model our Movies table!

Define a Movie model. In app.js, create a class named Movie that extends the Sequelize.Model base class:
...
Initialize a model. Call the static class init() method on the model name (Movie) to initialize and configure the model:
...
An important distinction is that the model name is singular and the table name is plural. Sequelize uses a library called inflection under the hood to automatically pluralize the table name (for example, from Movie to Movies).
Set Model Attributes and Options
The init() method takes two arguments. The first argument is an object literal that defines the model's attributes -- each attribute is a column of the table. The second argument is an object literal that sets the model options.

Now that we have a 'Movies' table, let's create a column for movie titles.

Set the model attributes. Define a column named 'title' by passing init() an object with a title property. Each column must have a data type. The title should be a string data type, so set the value of title to Sequelize.STRING:
...
Set the model options. You can set a number of options in your model definition; for example, a custom name for your model and a timestamp. The only required option is a sequelize property that defines the sequelize instance to attach to the model.

Pass init() an object as a second argument. Inside the object, attach the Sequelize instance by setting the sequelize property to the variable sequelize:
...
Inside the async function, await a Promise returned by calling Movie.sync():
...
Replace Movie.sync() with sequelize.sync():
...
Force sync all models. Pass { force: true } to the sync() method:
...
Insert data into the Movies table. Inside the try block, initialize a movie variable to a new Movie instance with await Movie.create():
...
Pass Movie.create() an object with a title property. Set the value to your favorite movie title as a string:
...
Log the data returned by movie.toJSON():
...
In your Terminal, run the app and create the record with npm start. You should see a similar JSON object in your console output:
